...
class Perturb:
    ...
    def add_typos(string, typos=1):
        ...
        """
        common_typo_map = {
            "or": "ro", "nd": "dn", "au": "ua","ing": "ign" ,"is":"si","ld":"dl","the":"teh","ith":"iht"
        }
        ret = []
        for p in common_typo_map.keys():
            if re.search(r'%s' % p, string):
                ret.append(re.sub(r'%s' %p, common_typo_map.get(p),string))
        if (len(ret)>1):
            rand = random.randint(0, len(ret)-1) #randommly select one typo
            return ret[rand]
        return ret
